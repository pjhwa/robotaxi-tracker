...
echo "- 아이폰/Tailscale : http://$(tailscale status --json 2>/dev/null | python3 -c "import sys,json; print(json.load(sys.stdin).get('Self',{}).get('DNSName','').rstrip('.'))" 2>/dev/null || echo jerrymacmini.tail487ba6.ts.net):4002"
echo "- 맥 로컬 HTTPS    : https://localhost:8443"
